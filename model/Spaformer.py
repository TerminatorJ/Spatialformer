...
class Spaformer(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx, *args, **kwargs):
        ...
        if self.outer_config["mini_batch"]:
            ...
            total_loss += (torch.exp(-self.log_sigma_class[0]) * Spa_loss + self.log_sigma_class[0])
        ...
        #         self.log('train_loss', MLM_loss, sync_dist=True, prog_bar=True, reduce_fx='mean')
        ...
        #     else:
        #         # MLM_loss = self.MEloss(mlm_predictions, real_indices, norm_exp) # MLM loss
        #         # Spa_loss = self.MDloss(spa_predictions, adjmtx)# spatial loss
        #         # total_loss = (torch.exp(-self.log_sigma_reg) * MLM_loss + self.log_sigma_reg + torch.exp(-self.log_sigma_class) * Spa_loss + self.log_sigma_class)
        #         self.log('train_MLM_loss', MLM_loss, sync_dist=True, prog_bar=False, reduce_fx='mean')
        #         self.log('train_Spa_loss', Spa_loss, sync_dist=True, prog_bar=False, reduce_fx='mean')
        #         self.log('train_total_loss', total_loss, sync_dist=True, prog_bar=True, reduce_fx='mean')
        ...
        return total_loss
    def linear_schedule_for_scale(self, num_stagnant_steps=2000):
        """
