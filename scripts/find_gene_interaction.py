...
class GeneInteractionProcessor:
    def __init__(self, threshold, radius, pair_threshold, number_cell, transcript_file, h5_file_path):
...
if __name__ == "__main__":
    ...
    h5_file_path = os.path.join(p_path, args.datapath_name, f"{args.dataname}_gene_interaction_{datetime.now()}.h5")
    #adding the partitions information
    
    h5_file_path = h5_file_path.split(".")[0] + "_" + str(args.partition) + "." + h5_file_path.split(".")[2]
    processor = GeneInteractionProcessor(args.threshold, args.radius, args.pair_threshold, args.number_cell, args.transcript_file, h5_file_path)
