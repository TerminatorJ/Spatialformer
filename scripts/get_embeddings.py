...
def load_model(config_path, ckp_path, bpp, ag_loss, nlayers, n_tasks):
    #loading the model
...
def get_repr(dataloader, objective):
    ...
    return embeddings, anns

def sample_dataset(my_dataset, fraction):
...
def workflow(my_dataset, fraction, batch_size, bpp, ag_loss, nlayers, n_tasks, objective, save_fra):
   
    print("saving the dataset to erda...")
    if save_fra:
        #save the dataset to the memory
        train_dataset = sample_dataset(my_dataset["train"], fraction)
        test_dataset = sample_dataset(my_dataset["test"], fraction)
        val_dataset = sample_dataset(my_dataset["validation"], fraction)
    ...
    # train_X, train_ann = getexpcount(train_dataset)
    # test_X, test_ann = getexpcount(test_dataset)
    # val_X, val_ann = getexpcount(val_dataset)
    # import pdb; pdb.set_trace()
    ...
    print("running train")
    train_embeddings, train_anns = get_repr(train_dataloader, objective)
    print("running test")
    test_embeddings, test_anns = get_repr(test_dataloader, objective)
    print("running val")
    val_embeddings, val_anns = get_repr(val_dataloader, objective)
...
if __name__ == "__main__":
    ...
    config, model = load_model("/home/sxr280/Spatialformer/config/_config_train.json", args.ckp_path, args.bpp, args.ag_loss, args.nlayers, args.n_tasks)
    #loading the dataset
    my_dataset = load_from_disk(os.path.join(hf_cache, args.dataset_path))
    workflow(my_dataset, args.fraction, args.batch_size, args.bpp, args.ag_loss, args.nlayers, args.n_tasks, args.objective, args.save_fra)
