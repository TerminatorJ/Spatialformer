...
def index2gene(filepath, qv = True):
    dataset = pd.read_csv(filepath)
    if qv:
        dataset = dataset[dataset['qv'] >= 20]
    dataset = dataset[~(dataset['feature_name'].str.startswith('Neg') | dataset['feature_name'].str.startswith('BLANK') | dataset['feature_name'].str.startswith('Unassigned'))]
...
def compute_edges_for_batch(args):
    batch_r_c, start_idx, kdtree, threshold = args  # Unpack the tuple
    edges_to_add = []
...
def build_graph_for_sample(data, threshold=10, batch_size=100, sample_id = None):
    # import pdb; pdb.set_trace()
    try:
        r_c = np.array(data[['x_location', 'y_location', 'z_location']])
    except:
        r_c = np.array(data[['x', 'y', "z"]])
    # import pdb; pdb.set_trace()
    ...
    for start_idx in tqdm(range(0, num_nodes, batch_size)):
        ...
        # Iterate over each element in batch_r_c
        for i, x in enumerate(batch_r_c, start=start_idx):
            # Measure time for finding neighbors
            neighbors_idx = kdtree.query_ball_point(x, threshold)
            G.add_edges_from((i, j) for j in neighbors_idx if i < j)
...
class GraphSAGEModel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log('train_acc', acc, sync_dist=True, reduce_fx='mean', prog_bar=True, batch_size=batch.x.size(0), on_step=False, on_epoch=True)
...
class MyTrainer:
    ...
    def make_callback(self):
        ...
        EarlyStopping(monitor = "train_acc", min_delta = 0.00, patience=5, verbose = True, mode = "max", stopping_threshold=0.83)
        ]
    ...
    def get_embedding(self, index_to_gene, ckp_path, token_path, input_dim, output_dim):
        '''
...
if __name__ == "__main__":
    ...
    # import pdb; pdb.set_trace()
    sample_files = [os.path.join(data_dir, file) for file in os.listdir(data_dir) if "transcripts_mod.csv" in file]
    index_to_gene, gene_to_index = index2gene(sample_files[0], qv = False)
    # import pdb; pdb.set_trace()
    if args.save_graph:
        #saving all the intermediate data
        # import pdb; pdb.set_trace()
        all_samples = [build_graph_for_sample(load_and_preprocess_data(sample_file), sample_id = "simulation") for sample_file in sample_files if f'subgraph_data_simulation.pt' not in os.listdir(data_dir)]
        # import pdb; pdb.set_trace()
    else:
        print("WARNNING: please make sure you have already save the graph for each sample")
        subgraphs = [torch.load(os.path.join(data_dir,f"subgraph_data_simulation.pt")) for sample_file in sample_files]
    ...
    import pdb; pdb.set_trace()
    embeddings = trainer.get_embedding(index_to_gene, "/home/sxr280/Spatialformer/output/GraphSAGE_model/GraphSAGE_model/checkpoints/step=0004000-train_loss=0.4470-val_loss=0.0000-train_acc=0.8339.ckpt", 32,
                                       "/home/sxr280/Spatialformer/tokenizer/token.json", config["output_dim"])
    pickle.dump(embeddings, open("/home/sxr280/Spatialformer/downstream/subcellular_localization_prediction/data/gene_embeddings_GraphSAGE.pkl", "wb"))
