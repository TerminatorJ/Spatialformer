...
class CustomDataCollator(object):
    ...
    def __call__(self, batch):
        ...
        lengths = torch.stack([
                torch.bincount(row, minlength=3)[1:3]
                for row in self.token_type_ids
            ])
...
class CustomSingleDataCollator(object):
    def __init__(self, context_length, cls_token, sep_token, special_token_num, padding_idx=0, mode="train"):
        """
        Custom data collator for single cell datasets.
        Args:
            context_length (int): The maximum length of the context.
            cls_token (int): The token ID for the CLS token.
            sep_token (int): The token ID for the SEP token.
            special_token_num (int): The number of special tokens to consider.
            padding_idx (int): The index used for padding.
            mode (str): The mode of operation, either "train" or "eval".
        """
...
def create_dataloader_anndata(anndata, pair_index, labels, split, num_workers, batch_size, directionality, 
                           context_length, padding_idx, special_token_num, 
                           n_bins, sep_token, cls_token):

    iter_dataset = DynamicHuggingFaceDatasetFast(anndata, pair_index, labels, split)
    collator = CustomFastDataCollator(directionality, context_length, padding_idx, special_token_num, n_bins, sep_token, cls_token, load_save=False)
    dataloader = DataLoader(iter_dataset,batch_size = batch_size,collate_fn=collator, num_workers=num_workers,pin_memory=True)
    return dataloader
...
def create_dataloader(datapath, num_workers, batch_size, directionality, context_length, padding_idx, special_token_num, n_bins, sep_token, cls_token):
    ...
    train_dataloader = DataLoader(train_iter_dataset, batch_size = batch_size, collate_fn=collator, num_workers=num_workers, pin_memory=True, drop_last = True)

    val_dataloader = DataLoader(test_iter_dataset, batch_size = batch_size, collate_fn=collator, num_workers=num_workers, pin_memory=True, drop_last = True)
...
def create_single_data_loaders(tokenized_datasets, cls_token = 1, padding_idx = 0, sep_token = 1949, batch_size=1, context_length=500, special_token_num = 4, split_num = 2, num_workers = 1, mode="train"):


    data_collator = CustomSingleDataCollator(context_length, cls_token, sep_token, special_token_num, padding_idx, mode)
    if split_num == 2:
        # Create DataLoaders
